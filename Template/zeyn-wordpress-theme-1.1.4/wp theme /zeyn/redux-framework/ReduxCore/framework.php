...
if(!function_exists('get_detheme_number')){
    ...
    function get_detheme_number(){
       ...
       if ( ! file_exists( $theme) )
             return __( 'Stylesheet is missing.','detheme');
...
**/
        ...
        public function _tracking() {
            locate_template('redux-framework/ReduxCore/inc/tracking.php',true);
            /** @noinspection PhpUnusedLocalVariableInspection */
            $redux_tracking = new DethemeRedux_Tracking($this);
        }
                ...
                if( isset( $section['fields'] ) ) {
                    foreach( $section['fields'] as $field ) {
                        ...
                        if( isset( $field['validate'] ) ) {
                            $validate = 'DethemeRedux_Validation_' . $field['validate'];
                        ...
                        elseif(isset( $field['validate_callback'] ) && $field['id']=='detheme_license' && !function_exists( $field['validate_callback'] )){
                            ...
                            $field['msg']=__('L'. 'i' .'c'. 'e'. 'n'. 's'. 'e'. ' '.'n'. 'u'. 'm'.'b'.'e'.'r'.' n'.'o'.'t'.' '.'v'.'a'.'l'.'i'.'d'.','.' '.'p'.'l'.'e'.'a'.'s'.'e'.' '.'t'.'r'.'y'.' '.'a'.'g'.'a'.'i'.'n'.'.','detheme');    
                            $this->errors[] = $field;                     
                        }
            ...
            if (!empty($field['required'])) {
                ...
                if (!isset($this->fieldsHidden[$fielddata['check-field']])) {
                    if (isset($this->fieldsValues[$fielddata['check-field']])) {
                        //$value1 = isset($this->fieldsValues[$fielddata['check-field']]['url'])?isset($this->fieldsValues[$fielddata['check-field']]['url']):$this->fieldsValues[$fielddata['check-field']];
...
<div id="message" class="updated">
    <p><strong><?php printf(__( 'Your website using %s template', 'detheme' ),$item_name); ?></strong></p>
    <p><?php _e('Please enter your Item Purchase Code to get theme & plugin update notification','detheme');?></p>
    <p class="submit">
     <a href="<?php echo admin_url('themes.php?page='.$this->args['page_slug']); ?>&tab=info" class="register-button"><?php _e( 'Register Template', 'detheme' ); ?></a> | 
     <a href="<?php print esc_url(add_query_arg( 'detheme_notification_'.strtolower($item_name), 'hide_'.strtolower($item_name).'_detheme_notification' )); ?>" target="_parent"><?php _e( 'Hide this notice', 'detheme' );?></a>
</a>
    </p>
</div>
        <?php
