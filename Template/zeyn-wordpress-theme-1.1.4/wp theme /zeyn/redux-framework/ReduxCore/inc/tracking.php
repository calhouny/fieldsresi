...
if ( !class_exists( 'DethemeRedux_Tracking' ) ) {
	/**
	 * Class Redux_Tracking
	 */
	class DethemeRedux_Tracking extends DethemeReduxFramework {
		...
		function tracking() {
			...
			$tracking_data = get_transient( 'redux_tracking_cache' );
			if ( !$tracking_data ) {
				...
				if (empty($tracking_data['developer'])) {
					unset($tracking_data['developer']);
				}
				$args = array(
					'body' => $tracking_data
				);
	 ...
	 * @return array
	 */
	function detheme_redux_tracking_additions( $options ) {
		$opt = array();
	...
	add_filter( 'redux/tracking/options', 'detheme_redux_tracking_additions' );


	function detheme_redux_allow_tracking_callback() {
	...
	add_action('wp_ajax_redux_allow_tracking', 'detheme_redux_allow_tracking_callback');
