...
while ( $my_query->have_posts() ) {
    ...
    if($this->getTeaserData('enable', $post->id) === '1') {
        ...
        $post->post_date=get_the_date('j M Y');
        $post->post_tags = get_the_tag_list(' ',', ','');
        $post->title_attribute = the_title_attribute('echo=0');

        $author_name=(''!=get_the_author_meta('nickname'))?get_the_author_meta('nickname'):get_the_author();
        $post->author ='<a href="' . esc_url( get_the_author_meta('url') ) . '" title="' . esc_attr( sprintf(__("Visit %s&#8217;s website",'detheme'), $author_name) ) . '" rel="author external">' . $author_name . '</a>';
        ...
        $post->excerpt = $this->getPostExcerpt();
        $post->thumbnail_data = $this->getPostThumbnail($post->id, $grid_thumb_size);
        $post->grid_thumb_size = $grid_thumb_size;
        $post->thumbnail = $post->thumbnail_data && isset($post->thumbnail_data['thumbnail']) ? $post->thumbnail_data['thumbnail'] : '';
...
?>
<div class="post-masonry <?php echo apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->settings['base']) ?>">
    <div class="wpb_wrapper">
        ...
        <div class="teaser_grid_container">
            ...
            <ul class="wpb_thumbnails wpb_thumbnails-fluid vc_clearfix" data-layout-mode="<?php echo $grid_layout_mode ?>">
                ...
                $template_path="vc_templates/post_block/post.php";



                ?>
                <?php foreach($posts as $key => $vc_post): 

                    global $post;

                    $post=$vc_post;
                    $post->link_target=$grid_link_target;
                ...
                <li class="isotope-item <?php echo apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li').$post->categories_css ?>"<?php echo $block_style ?>>
                    <div class="isotope-inner <?php print $post->post_format;?>">
                        ...
                        locate_template(array('vc_templates/post_block/post-'.$post->post_format.'.php',$template_path),true,false);
                        ?>
                    </div>
